...
class ResNet(nn.Module):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
                 num_classes: int, nf: int, first_kernel_size=3, hookme=False, dim=512, pred_dim=128) -> None:
        """
    ...
    def forward(self, x: torch.Tensor, returnt='out') -> torch.Tensor:
        ...
        if returnt == 'out':
            ...
            p = self.predictor(feature) # NxC
            # TODO: not hard code that
            return out, feature, p, feature #.detach()
        else:
            return (out, feature)
...
def resnet50(nclasses: int, nf: int = 64, first_k=3, hookme=False) -> ResNet:
    """
    ...
    :return: ResNet network
    """
    return ResNet(Bottleneck, [3, 4, 6, 3], nclasses, nf, first_kernel_size=first_k, hookme=hookme)
...
def test():
    net = resnet18(nclasses=100)
    y, out_list = net(torch.randn(1,3,32,32), returnt='full')
    print(y.size(), len(out_list))
