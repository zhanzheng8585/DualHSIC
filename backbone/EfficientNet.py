...
feature_nums = {
    'efficientnet-b2': [1, 4, 7, 11, 15, 20]#, 22]
}
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, dim=1408, pred_dim=352):
        super().__init__()
    ...
    def extract_features(self, inputs):
        ...
        full_feats = []
        # Stem
        prerelu = self._bn0(self._conv_stem(inputs))
        stem_x = self._swish(prerelu)
        x = stem_x
        full_feats.append(stem_x if not self.return_prerelu else prerelu)
        # Blocks
        for idx, block in enumerate(self._blocks): 
            drop_connect_rate = self._global_params.drop_connect_rate
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / len(self._blocks)  # scale drop connect_rate
            x = block(x, drop_connect_rate=drop_connect_rate)
            if idx in feature_nums[self.model_name]:
                full_feats.append(x)
                if not self.return_prerelu and not hasattr(self, 'warned'):
                    self.warned = True
                    print('WARNING: post-relu features are not defined for effnet')
        # Head
        prerelu = self._bn1(self._conv_head(x))
        x = self._swish(prerelu)
        full_feats.append(x if not self.return_prerelu else prerelu)
    ...
    def set_return_prerelu(self, enable=True):
        ...
        for c in self.modules():
            if isinstance(c, MBConvBlock):
                c.return_prerelu = enable
        
    # @autocast()
    def features(self, x: torch.Tensor, returnt='out'):
        assert returnt in ['out']
    ...
    def forward(self, inputs, returnt='out'):
        ...
        """
        # Convolution layers
        x, feat_list = self.extract_features(inputs)
        ...
        if returnt == 'out':
            ...
            return x, feat_list
        elif returnt == 'use_siam':
            p = self.predictor(feats) # NxC
            # TODO: not hard code that
            return x, feats, p, feats #.detach()
        else:
            return (x, feats)
    ...
    def from_name(cls, model_name, in_channels=3, **override_params):
        ...
        model.num_classes = override_params['num_classes']
